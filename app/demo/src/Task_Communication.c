...
// hi_u8    Uart_Send_buff[UART_BUFF_SIZE];
// int Communication_num = 0; // Debug
// Motor_status_enum Motor_Left_Status = MOTOR_DOWN_DOG_BACK, Motor_Right_Status = MOTOR_DOWN_CAT_BACK, Motor_Up_Status = MOTOR_UP_HORIZONTAL;
...
//     communication_attr.stack_size = 4096;
//     communication_attr.task_prio = 28;
...
//     hi_u32 ret;
//     while(1) {
//         memset_s(Uart_Send_buff, sizeof(Uart_Send_buff)/sizeof(Uart_Send_buff[0]), 0x0, sizeof(Uart_Send_buff)/sizeof(Uart_Send_buff[0]));
//         if (Uart_Receive_Flag) {
//             /*接收数据*/
//             printf("Entry Rec");
//             hisignalling_msg_receive(Uart_Rec_Buffer, Uart_Rec_Buff_len);
//             /*回显数据组包*/
//             if (Uart_Rec_Buff_len > (HISGNALLING_MSG_CRC32_LEN + HISIGNALLING_MSG_HEADER_TAIL_LEN)) {
//                 memcpy_s(Uart_Send_buff, (Uart_Rec_Buff_len - HISGNALLING_MSG_CRC32_LEN - HISIGNALLING_MSG_HEADER_TAIL_LEN), \
//                 &Uart_Rec_Buffer[HISGNALLING_MSG_FRAME_HEADER_LEN], \
//                 (Uart_Rec_Buff_len - HISGNALLING_MSG_CRC32_LEN - HISIGNALLING_MSG_HEADER_TAIL_LEN));
//                 /*接收到Hi3516DV300数据后，发送回显*/
//                 hisignalling_msg_send(Uart_Send_buff, (Uart_Rec_Buff_len - HISGNALLING_MSG_CRC32_LEN - HISIGNALLING_MSG_HEADER_TAIL_LEN));
//             }
//             Uart_Receive_Flag = HI_FALSE;
//             memset_s(Uart_Rec_Buffer, UART_BUFF_SIZE, 0x0, UART_BUFF_SIZE);
//         }
//         hi_sleep(HISGNALLING_FREE_TASK_TIME);
//     }
//     ret = hi_task_delete(task_communication_id);
//     if (ret != HI_ERR_SUCCESS) {
//         HISIGNALLING_LOG_ERROR("Failed to delete task communication %d\r\n", ret);
//     }
...
//         case PET_CAT_TIC:
//         case PET_DOG_TIC:
//             Motor_Right_Status = MOTOR_DOWN_CAT_BACK;
//             Motor_Left_Status = MOTOR_DOWN_DOG_BACK;
//             Motor_Up_Status = MOTOR_UP_HORIZONTAL;
//         break;
//         case PET_CAT_MIC_OUT:
//             printf("*****  3 ******\n");
//             Motor_Right_Status = MOTOR_DOWN_CAT_BACK;


//             // Motor_Right_Status = MOTOR_DOWN_CAT_OUT;
//             // Motor_Up_Status = MOTOR_UP_HORIZONTAL;
//         break;
//         case PET_DOG_MIC_OUT:
//             printf("*****  4 ******\n");
//             Motor_Right_Status = MOTOR_DOWN_CAT_OUT;

//             // Motor_Left_Status = MOTOR_DOWN_DOG_OUT;
//             // Motor_Up_Status = MOTOR_UP_HORIZONTAL;
//         break;
//         case PET_CAT_MIC_OUT_DOWN:
//             printf("*****  5 ******\n");

//             Motor_Right_Status = MOTOR_UP_HORIZONTAL;
//         // **** 这里还应该家上压力传感器的回传判断 **** //
//             if(Motor_Up_Status != MOTOR_UP_DOG && 1) // 当前为水平
//                 Motor_Up_Status = MOTOR_UP_CAT;
//         break;
//         case PET_DOG_MIC_OUT_DOWN:
//             Motor_Left_Status = MOTOR_DOWN_DOG_OUT;
//             if(Motor_Up_Status != MOTOR_UP_CAT && 1)
//                 Motor_Up_Status = MOTOR_UP_DOG;
//         break;
//         default:
//         break;   
//     }
